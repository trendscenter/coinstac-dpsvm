...
def noisevector(dim, scale_lambda):
    ...
    # 1. The distribution of norm is Erlang with parameters (d, lambda).
    # 2. For the direction, we can pick uniformly by sampling d i.i.d. Gaussians and normalizing them.
    
    # generate norm
    normn = np.random.gamma(dim, 1/scale_lambda, 1) 
    # generate direction
    r1 = np.random.normal(0, 1, dim)
    n1 = np.linalg.norm(r1, 2)  # get the 2-norm of r1
    r2 = r1 / n1  # normalize r1
    # get the result noise vector
    res = r2 * normn 
    return res

def huber(z, h):  #chaudhuri2011differentially corollary 12
...
def svm_output_train(data, labels, epsilon, Lambda, h):
    ...
    f = minimize(obj_func, x0, method='Nelder-Mead').x
    fpriv = f + noise
    return fpriv
...
def svm_objective_train(data, labels,  epsilon, Lambda, h):
    ...
    if Epsilonp > 0:
        ...
        Epsilonp = epsilon / 2
    noise = noisevector(l, Epsilonp/2)
